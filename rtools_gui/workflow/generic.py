    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                ram = int(imager.match('Found (\\d+) MiB RAM')[0].decode())
                self.set_progress(0.5)
                real_serial_number = int(imager.match('Serial Number: ([0-9A-Fa-f]+)')[0].decode(), 16)
                self.set_progress(0.6)
                real_board_type = imager.match('Board type: \\d+ \\(([0-9A-Za-z ]+)\\)')[0].decode()
                self.set_progress(0.65)
                real_board_version = int(imager.match('Board version: (\\d+)')[0].decode())
                self.set_progress(0.7)
                real_first_mac = b':'.join(imager.match('MAC address: ([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})')).decode()
                self.set_progress(0.8)
                public_key = imager.match('ECDSA Public Key: (0[23][0-9A-Fa-f]{132})')[0].decode()
